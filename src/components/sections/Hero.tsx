...
import { ArrowRight, FileText, Radio, Server, Cpu, Database } from "lucide-react";
import { profile } from "../../lib/profile";
import { getDevUptimeStats } from "../../lib/devStats";
import OperatorAvatar from "../ui/OperatorAvatar";

const BOOT_LOGS = [
  { text: `PRWLR_SYS: Booting kernel ${profile.systemVersion}...`, type: "info" },
  { text: "SECURE_UPLINK: Est. TLS handshakes verified.", type: "success" },
  { text: "MODULE_LOAD: React 19.x & Tailwind CSS v4... OK", type: "success" },
  { text: "MODULE_LOAD: Motion 12.x hardware rendering... OK", type: "success" },
  { text: "DECRYPTING: Operator credentials decryption loop...", type: "info" },
  { text: `IDENTITY_LOAD: Operator ${profile.realName} loaded.`, type: "success" },
  { text: "SYS_STATUS: Hypervisor active. Welcome back operator.", type: "warn" }
];
...
export default function Hero() {
  ...
  const bootLogs = BOOT_LOGS;
  const devStats = getDevUptimeStats();

  const isBooted = bootStep >= bootLogs.length;

  // Boot logs rendering loop
  useEffect(() => {
    if (isBooted) {
      return;
    }
  ...
  return (
    <section id="home" className="py-8 md:py-16 w-full flex flex-col gap-10 select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch pt-2">
        ...
        {/* Live Hacker Terminal Module (Right Side Panels) */}
        <div className="col-span-12 md:col-span-4 flex flex-col justify-center gap-4">
          ...
          />
          <div className="border border-neon-lime/20 bg-black/75 rounded-sm p-4 w-full h-80 font-mono text-xs sm:text-[13px] leading-relaxed flex flex-col shadow-inner neon-border-lime">
            {/* Terminal Header */}
            <div className="flex justify-between items-center border-b border-neon-lime/15 pb-2 mb-3">
              <div className="flex items-center gap-1.5">
                <span className="w-2.5 h-2.5 rounded-full bg-red-500/80"></span>
                <span className="w-2.5 h-2.5 rounded-full bg-neon-lime/80"></span>
                <span className="w-2.5 h-2.5 rounded-full bg-green-500/80"></span>
              </div>
              <div className="text-gray-300 text-[11px] flex items-center gap-1">
                <Server className="w-3 h-3 text-neon-lime/60" />
                <span>TERMINAL_OUTPUT.INI</span>
              </div>
            </div>

            {/* Living Boot Log Console Output */}
            <div className="grow overflow-y-auto pr-1 flex flex-col gap-2 font-mono scrollbar-thin">
              {bootLogs.slice(0, bootStep).map((log, index) => {
                ...
                return (
                  <motion.div
                    ...
                    key={index}
                    className={`${colorClass} flex gap-2 font-mono`}
                  >
                    <span className="text-gray-300 font-bold font-mono">{prefix}</span>
              ...
              {bootStep < bootLogs.length && (
                <div className="flex items-center gap-2 text-neon-lime font-mono animate-pulse">
                  <span className="text-neon-lime/40 font-mono">[EXEC]</span>
                  <span className="font-mono">Processing uplink secure sequence...</span>
                </div>
              )}
            </div>

            {/* Live Stats Diagnostics Bar at absolute bottom of terminal */}
            <div className="border-t border-neon-lime/10 pt-2.5 mt-2 flex justify-between text-[11px] text-gray-300 font-mono">
              <span className="flex items-center gap-1">
                <Database className="w-3 h-3 text-neon-purple" />
                <span>MEMORY: <span className="text-neon-purple">STABLE</span></span>
              </span>
              <span className="flex items-center gap-1">
                <Cpu className="w-3 h-3 text-neon-blue" />
                <span>CPU: <span className="text-neon-blue">0.14%</span></span>
              </span>
            </div>
          </div>
        </div>
